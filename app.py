...
# # example usage inside some module
# from config import GROQ_API_KEY
# # use GROQ_API_KEY as before (it will be None if not set)
...
# # Header with centered logo ABOVE the title
# st.markdown("""
#     <style>
#         .logo-wrapper {
#             width: 100%;
...
# try:
#     st.image("finance.jpg", width=150)
# except:
#     st.warning("⚠️ Logo image not found. Make sure finance.jpg is in the same folder as app.py")

# st.markdown("</div>", unsafe_allow_html=True)

# # Title BELOW logo
# st.markdown("<p class='header-quote-custom'>FINWISE: Finance Assistant</p>", unsafe_allow_html=True)
...
#     # Local paragraph fallback generator (guaranteed paragraph-style)
#     def generate_paragraph_fallback(question: str, lang_code: str) -> str:
#         q = question.strip()
#         paragraph = ""
#         q_lower = q.lower()
#         if any(kw in q_lower for kw in ["what is", "define", "meaning of", "kya hai", "shu chhe", "explain"]):
#             paragraph = (
#                 f"A mutual fund is a pooled investment vehicle that collects money from multiple investors to invest in a diversified portfolio of securities such as stocks, bonds, or other assets. "
#                 f"Professional fund managers run the fund according to its stated objectives, and the value of the investor's holding is represented by the Net Asset Value (NAV) which changes with market movements. "
#                 f"Mutual funds offer diversification, professional management, and liquidity, but they carry market risk and fees that investors should consider when choosing a fund."
#             )
#         elif any(kw in q_lower for kw in ["sip", "systematic investment plan", "how to start sip", "start sip"]):
#             paragraph = (
#                 f"A Systematic Investment Plan (SIP) is a method of investing a fixed amount of money at regular intervals into a mutual fund, typically an equity or hybrid fund. "
#                 f"By investing periodically, SIPs help average purchase cost over time and reduce the risk of poor timing; they are suitable for investors with a long-term horizon who prefer disciplined investing. "
#                 f"To start an SIP, decide on the fund and the amount, set a frequency (monthly is common), and either use your broker or the fund house portal to schedule auto-debits."
#             )
#         else:
#             paragraph = (
#                 f"Regarding '{q}': here is a concise explanation and practical guidance — start by clarifying your objective, time horizon and risk tolerance, then choose products that match these criteria. "
#                 f"Consider costs, tax implications and liquidity, and when in doubt begin with a small allocation or a systematic approach to reduce timing risk. "
#                 f"If you need tailored recommendations, consult a certified financial advisor who can evaluate your full financial picture."
#             )
#         paragraph = " ".join(paragraph.split())
#         if lang_code != "en":
#             try:
#                 return translate_text(paragraph, lang_code)
#             except Exception:
#                 return paragraph
#         return paragraph

#     # A helper to check if the model answer is unhelpful (short/echo)
#     def is_unhelpful_answer(ans: str, question: str) -> bool:
#         if not ans or len(ans.strip()) < 30:
#             return True
#         a = ans.strip().lower()
#         q = question.strip().lower()
#         if a == q or a.startswith(q) or (q in a and len(a.split()) <= len(q.split()) + 3):
#             return True
#         return False

#     # Try Groq first (if API key present), otherwise use multilingual_chatbot, then fallback
#     def ask_groq(question: str, lang_code: str) -> str:
#         try:
...
#             # Instruction: single paragraph, no bullets, include one practical tip
#             instr = (
#                 "You are a helpful financial assistant. Provide a single, well-formed paragraph (no bullet lists) that answers the user's question clearly, "
#                 "includes one short practical tip, and avoids repeating the user's question verbatim. Keep it professional and easy to understand."
#             )
#             # If translation needed, ask in English and translate after
#             if lang_code != "en":
...
#                     max_tokens=400
#                 )
#                 en_answer = completion.choices[0].message.content.strip()
#                 return translate_text(en_answer, lang_code)
#             else:
#                 prompt = f"{instr}\n\nQuestion: {question}"
#                 completion = client.chat.completions.create(
#                     model=model_name,
...
#                 return completion.choices[0].message.content.strip()
#         except Exception as e:
#             logging.warning(f"Groq call failed or unavailable: {e}")
#             return None

#     # Use your existing multilingual_chatbot as a secondary model
#     def ask_multilingual_chatbot_with_instruction(question: str, lang_code: str) -> str:
#         instruct = "Answer in one single paragraph, be clear and helpful, include one short practical tip, and avoid repeating the question."
#         try:
#             if lang_code == "en":
#                 return multilingual_chatbot(f"{instruct}\n\n{question}")
#             else:
#                 # translate to English, ask, then translate back
#                 en_q = translate_text(f"{instruct}\n\n{question}", "en")
#                 en_ans = multilingual_chatbot(en_q)
#                 return translate_text(en_ans, lang_code)
#         except Exception as e:
#             logging.warning(f"multilingual_chatbot failed: {e}")
#             return None
...
#         else:
#             # 1) Try Groq (strong preference when key provided)
#             answer = ask_groq(user_question, lang_code)

#             # 2) If Groq missing/unhelpful, try multilingual_chatbot with instruction
#             if answer is None or is_unhelpful_answer(answer, user_question):
#                 answer = ask_multilingual_chatbot_with_instruction(user_question, lang_code)

#             # 3) If still unhelpful, try one more time with an explicit 'detailed' prompt to chatbot
#             if answer is None or is_unhelpful_answer(answer, user_question):
#                 try:
#                     extra_prompt = f"Please provide a detailed single-paragraph answer and one practical tip: {user_question}"
#                     if lang_code == "en":
#                         answer = multilingual_chatbot(extra_prompt)
#                     else:
#                         en_extra = multilingual_chatbot(translate_text(extra_prompt, "en"))
#                         answer = translate_text(en_extra, lang_code)
#                 except Exception:
#                     answer = None

#             # 4) Final fallback: local generator that ALWAYS returns a paragraph
#             if answer is None or is_unhelpful_answer(answer, user_question):
#                 answer = generate_paragraph_fallback(user_question, lang_code)

#             # Ensure single paragraph formatting
#             final_answer = " ".join(answer.split())

#             # Display result
#             st.markdown("**Answer:**")
#             st.markdown(f"<div class='highlight'>{final_answer}</div>", unsafe_allow_html=True)
...
#             # same flow as text
#             answer = ask_groq(voice_text, lang_code)
#             if answer is None or is_unhelpful_answer(answer, voice_text):
#                 answer = ask_multilingual_chatbot_with_instruction(voice_text, lang_code)
#             if answer is None or is_unhelpful_answer(answer, voice_text):
#                 try:
#                     extra_prompt = f"Please provide a detailed single-paragraph answer and one practical tip: {voice_text}"
#                     if lang_code == "en":
#                         answer = multilingual_chatbot(extra_prompt)
#                     else:
#                         en_extra = multilingual_chatbot(translate_text(extra_prompt, "en"))
...
#                     answer = None
#             if answer is None or is_unhelpful_answer(answer, voice_text):
#                 answer = generate_paragraph_fallback(voice_text, lang_code)
